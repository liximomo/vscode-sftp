...
import { sync, TransferDirection, SyncModel } from '../transfer';
...
describe('transfer algorithm', () => {
  describe('sync', () => {
    ...
    test('sync with SyncModel "FULL"', async () => {
      mockFs({
        local: {
          a: file('a', 1),
          b: file('b', 1),
    ...
    test('sync with SyncModel "UPDATE"', async () => {
      mockFs({
        local: {
          a: file('a', 1),
          b: file('b', 1),
    ...
    test('sync with SyncModel "BOTH DIRECTION"', async () => {
      mockFs({
        local: {
          a: file('a', 1),
          b: file('b', 1),
      ...
      const deleted = await sync(
        {
          ...
          transferDirection: TransferDirection.LOCAL_TO_REMOTE,
          transferOption: {
            model: SyncModel.BOTH_DIRECTIONS,
            perserveTargetMode: false,
          },
        },
        collect
